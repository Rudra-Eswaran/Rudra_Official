...
import { SiSelenium, SiSpringboot, SiCanva } from "react-icons/si";
...
const About = () => {
  ...
  return (
    <div
      id="about"
      className="bg-blue-600 min-h-screen flex flex-col md:flex-row items-center justify-center px-4 sm:px-6 md:px-8 py-16 gap-12 overflow-x-hidden max-w-full box-border"
    ...
    >
      <motion.div
        ...
        className="w-full max-w-full md:max-w-lg text-white text-center md:text-left box-border"
      >
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        transition={{ duration: 1 }}
        className="w-full max-w-full md:max-w-xl box-border"
      >
        <h2 className="text-3xl font-semibold text-white mb-6 text-center md:text-left max-w-full break-words">
          Skills
        </h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 max-w-full box-border">
          {skillsWithIcons.map(({ name, icon }, index) => (
            <div
              key={name}
              data-aos={index % 2 === 0 ? "fade-right" : "fade-left"}
              data-aos-delay={index * 100}
              className="bg-white bg-opacity-20 text-white rounded-xl py-3 px-4 flex items-center gap-2 justify-center font-medium hover:bg-opacity-40 transition duration-300 max-w-full box-border"
