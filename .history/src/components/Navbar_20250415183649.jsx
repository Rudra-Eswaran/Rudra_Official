  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-10 py-4 transition-all duration-300 ease-in-out ${
        scrolled ? "bg-teal-800 shadow-lg" : "bg-teal-900"
      }`}
    >
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <ul className="md:hidden bg-teal-800 text-white p-6 space-y-4">
          <li>
            <Link
              to="/"
              onClick={() => setIsOpen(false)}
              className="block text-lg hover:text-teal-400 transition-all duration-300"
            >
              Home
            </Link>
          </li>
