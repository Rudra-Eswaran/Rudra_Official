      ...
      <div className="container-fluid px-4 md:px-6 mx-auto flex justify-between items-center">
        ...
        <ul className="hidden md:flex space-x-8 text-lg font-medium text-blue-800">
          <li>
            <Link
              to="/"
              className="hover:text-blue-600 transition duration-300 ease-in-out transform hover:scale-105"
            >
              Home
            </Link>
          </li>
          ...
          <li className="relative group">
            ...
            <ul className="absolute left-0 hidden mt-2 space-y-2 bg-white shadow-lg text-blue-800 p-4 rounded-lg w-48 group-hover:block">
              <li>
                <Link
                  to="/products"
                  className="block text-sm hover:text-blue-600 transition duration-300 ease-in-out"
                >
                  Products
                </Link>
              </li>
      ...
      {isOpen && (
        <ul className="md:hidden bg-white text-blue-800 p-6 space-y-4">
          ...
          <li className="relative">
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="block text-lg hover:text-blue-600 transition-all duration-300 w-full text-left"
            >
              Services
            </button>
            {isOpen && (
              <ul className="space-y-2 pl-4">
                <li>
                  <Link
                    to="/products"
                    onClick={() => setIsOpen(false)}
                    className="text-sm hover:text-blue-600 transition duration-300 ease-in-out"
                  >
                    Products
                  </Link>
                </li>
