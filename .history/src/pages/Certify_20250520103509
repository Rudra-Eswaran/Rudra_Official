...
// Helper: format folder names to neat UI labels
...
const Certify = () => {
  ...
  useEffect(() => {
    // Dynamically load images from public folder structure
    const loadCertificates = () => {
      const folder = activeCategory;
      // Assume max 10 certificates per category max
      const images = [];
      for (let i = 1; i <= 10; i++) {
        const path = `/Certificates/${folder}/C${i}.jpg`;
        // Since no file system check here, just add all and rely on img error fallback
        images.push(path);
      }
  ...
  return (
    <div className="min-h-screen bg-[#99bbff] p-6 flex flex-col items-center">
