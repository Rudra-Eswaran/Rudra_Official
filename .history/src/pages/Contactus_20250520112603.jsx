...
const Contactus = ({ scrollToFooter }) => {
  ...
  const [isOverlayVisible, setIsOverlayVisible] = useState(false); // Control overlay visibility

  useEffect(() => {
    AOS.init({
      duration: 1000,
      easing: 'ease-in-out',
      once: false, // Set to false so animation will be triggered on every scroll into view
    });
  }, []);
    ...
    >
      <div className="relative container mx-auto max-w-5xl text-center">
        <motion.h2
          ...
          data-aos="fade-up" // Animation added to this header
        >
          Contact Us
        </motion.h2>
      </div>
      ...
      <div className="relative container mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl pb-0 mb-0">
        ...
        <div className="space-y-6" data-aos="fade-right">
          {Object.keys(CONTACT_INFO).map((key) => (
            <InfoBox
              ...
              data-aos="fade-left" // Animation added to each info box
            />
          ))}
        </div>
        ...
        <div className="bg-blue-500 bg-opacity-90 p-6 md:p-8 rounded-lg shadow-lg" data-aos="fade-left">
          <motion.h3
            ...
            data-aos="fade-up" // Animation for form header
          >
            Get in Touch
          </motion.h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField type="text" name="name" placeholder={FORM_FIELDS.name} value={formData.name} handleChange={handleChange} data-aos="fade-up" />
              <InputField type="email" name="email" placeholder={FORM_FIELDS.email} value={formData.email} handleChange={handleChange} data-aos="fade-up" />
            </div>
            <InputField type="tel" name="phone" placeholder={FORM_FIELDS.phone} value={formData.phone} handleChange={handleChange} data-aos="fade-up" />
            <TextAreaField name="message" placeholder={FORM_FIELDS.message} value={formData.message} handleChange={handleChange} data-aos="fade-up" />
            <motion.button
      ...
      {/* Show Success or Failure Messages as overlay */}
      {isOverlayVisible && (
...
const InfoBox = ({ icon, title, text, onClick }) => (
  <motion.div
    ...
    data-aos="fade-up" // Animation for each info box
  >
    <motion.div
...
const InputField = ({ type, name, placeholder, value, handleChange }) => (
  <motion.input
    ...
    data-aos="fade-up" // Animation for input fields
  />
);
...
const TextAreaField = ({ name, placeholder, value, handleChange }) => (
  <motion.textarea
    ...
    onChange={handleChange}
    className="w-full p-3 md:p-4 rounded bg-gray-200 border border-gray-300 focus:border-blue-500 transition"
    data-aos="fade-up" // Animation for textarea fields
  />
);
