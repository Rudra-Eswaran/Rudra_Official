...
const certificateMap = {
  aicte: 10, aws: 10, cisco: 10, coursera: 10, "ec-council": 10, geekstar: 10,
  guvi: 10, infosys: 10, internshipala: 10, iste: 10, linkedin: 10, microsoft: 10,
  nptel: 10, others: 10, scaler: 10, tcs: 10, tryhackme: 10, udemy: 10, zscaler: 10,
};
...
const Certify = () => {
  const categories = Object.keys(certificateMap);
  ...
  useEffect(() => {
    const loadCertificates = () => {
      const count = certificateMap[activeCategory] || 0;
      const images = Array.from(
        { length: count },
        (_, i) => `/Certificates/${activeCategory}/C${i + 1}.jpg`
      );
  ...
  return (
    <div className="min-h-screen bg-[#3399ff] p-6 flex flex-col items-center">
      <h2 className="text-4xl font-bold text-white mb-6 select-none">Certificates</h2>
      ...
      <div className="relative w-full max-w-3xl h-[450px] bg-white rounded-lg shadow-lg overflow-hidden select-none">
        <AnimatePresence mode="wait">
          {certificates.length > 0 ? (
            ...
            <motion.p
              className="text-center text-gray-700 text-xl mt-20"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            >
