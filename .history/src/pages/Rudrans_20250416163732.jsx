...
const Rudrans = () => {
  ...
  return (
    <section id="team" className="relative w-full min-h-screen bg-gray-100">
      ...
      <button
        onClick={handlePrev}
        className="absolute left-4 top-1/2 transform -translate-y-1/2 text-4xl text-gray-800 bg-white p-2 rounded-full shadow-lg hover:bg-gray-200 transition"
      >
        &#8592;
      </button>
      ...
      <div className="w-full h-full overflow-hidden">
        ...
        >
          {teamData.map((member, index) => (
            <div
              key={index}
              className="w-full h-screen flex flex-col items-center justify-center bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
            >
              <img
                src={member.photo}
                alt={member.name}
      ...
      <button
        onClick={handleNext}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 text-4xl text-gray-800 bg-white p-2 rounded-full shadow-lg hover:bg-gray-200 transition"
      >
        &#8594;
      </button>
    </section>
