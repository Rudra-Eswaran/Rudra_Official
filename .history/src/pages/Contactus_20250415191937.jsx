...
import { CONTACT_INFO, MODAL_MESSAGES, FORM_FIELDS, BUTTON_LABELS } from "../Constants";
...
const Contactus = ({ scrollToFooter }) => {
  ...
  return (
    <motion.section
      ...
      className="relative text-black pt-24 pb-12 px-4 md:px-6 min-h-screen bg-white"
    >
      ...
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
        ...
        <div className="bg-blue-500 bg-opacity-90 p-6 md:p-8 rounded-lg shadow-[0_4px_20px_rgba(0,123,255,0.3)]">
          <motion.h3 whileHover={{ scale: 1.05 }} className="text-2xl font-semibold mb-4 text-white">
            Get in Touch
          </motion.h3>
...
const InfoBox = ({ icon, title, text, onClick }) => (
  <motion.div
    onClick={onClick}
    className="cursor-pointer bg-white p-4 md:p-6 rounded-lg flex items-center gap-4 shadow-md hover:shadow-lg transition-transform hover:scale-105"
    whileHover={{ boxShadow: "0px 0px 10px rgba(0, 123, 255, 0.4)" }}
  >
    <div className="w-12 h-12 flex items-center justify-center rounded-full bg-blue-600 text-white shadow-lg">
      {icon}
    </div>
    <div>
...
const InputField = ({ type, name, placeholder, value, handleChange }) => (
  <motion.input
    ...
    className="w-full p-3 rounded bg-gray-200 border border-gray-300 focus:border-blue-500 transition"
  />
);
