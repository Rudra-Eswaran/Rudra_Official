...
const Certify = () => {
  ...
  return (
    <div className="min-h-screen bg-black p-6 flex flex-col items-center">
      <h2 className="text-4xl font-bold uppercase text-white mb-6 select-none">
        Certificates
      </h2>

      {/* Category Buttons Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-5xl mb-6 w-full px-4">
        {certificateCategories.map((cat) => (
          <button
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={`border-2 border-black rounded-md font-bold uppercase text-sm p-2 text-center break-words leading-tight h-[60px] overflow-hidden transition-colors duration-300
              ${
                activeCategory === cat
