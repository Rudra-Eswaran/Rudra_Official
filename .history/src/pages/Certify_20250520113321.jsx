...
const totalCertificates = Object.values(certificateMap).reduce(
  (acc, count) => acc + count,
  0
);
...
const Certify = () => {
  ...
  useEffect(() => {
    // Init AOS with mirror:true and once:false for animation reverse on scroll out
    AOS.init({
      duration: 1000,
      easing: "ease-in-out",
      once: false,
      ...
      for (let i = 1; i <= numCertificates; i++) {
        try {
          const img = await import(
            `../assets/Certificates/${activeCategory}/C${i}.jpg`
          );
  ...
  return (
    <div
      className="w-full min-h-screen flex flex-col items-center justify-center bg-blue-700 text-white p-4"
      data-aos="fade-up"
