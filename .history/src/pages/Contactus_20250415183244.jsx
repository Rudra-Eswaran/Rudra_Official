...
import locationAnimation from "../animations/location.json";  // Assuming these JSON files are in your project folder.
...
const Contactus = ({ scrollToFooter }) => {
  ...
  return (
    <motion.section
      ...
      className="relative text-black py-8 px-4 md:py-10 md:px-6 min-h-screen md:min-h-fit"
      style={{
      ...
      <div className="relative container mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl pb-0 mb-0">
        <div className="space-y-6">
          {Object.keys(CONTACT_INFO).map((key) => (
            <InfoBox
              key={key}
              icon={key === "location" ? <Lottie animationData={locationAnimation} loop={true} className="w-12 h-12" /> : 
                key === "phone" ? <Lottie animationData={phoneAnimation} loop={true} className="w-12 h-12" /> : 
                <Lottie animationData={emailAnimation} loop={true} className="w-12 h-12" />}
